...
// 직렬화 : 프로그램에서 사용되는 객체를 파일이나 네트워크로 전송 하기 위해서 연속적인 바이트 형태로 변경하는 것
...
public class SerialStreamEx1 {
    ...
    static void writeList() throws IOException {
        ...
        list.add(new Board(1,"일타스캔들","로맨스 코미디","유제원",new Date()));
    ...
    static List<Board> readList() throws IOException, ClassNotFoundException {
        ...
        List<Board> list = (List<Board>) ois.readObject(); //파일의 바이너리를 읽어서 객체를 역직렬화해줌
        return list;
    }
}
