...
public class AirCon {
    ...
    public AirCon(){
        ...
        Calendar now = Calendar.getInstance(); //추상클래스 객체로안만들어짐
    ...
    //에어컨의 대해 온도와 바람세기 설정, 설정 이후 조건에 따라 히터 또는 쿨러를 켬
    public void setAirConState() {
        ...
        if(currTemp > setTemp) { //현재가 더운 상태
            ...
            isCooler=true;
            isHeater=false;
        }else if(currTemp < setTemp) {// 현재가 추운상태
    ...
    public void setPower(boolean power) {//전원키기
        isPower = power;
    }
