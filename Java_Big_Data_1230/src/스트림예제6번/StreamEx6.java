...
//스트림 최종 연산 : 중개연산을 통해 변환된 스트림은 마지막으로 최종 연산을 통해 요소를 소모해 결과를 출력
public class StreamEx6 {
    public static void main(String[] args) {
        ...
        //reduce() : 첫번째 요소와 두번째 요소를 가지고 연산을 수행 후, 그결과의 세번째요소를 가지고 다시 연산
        //문자열은 이어붙이기 숫자는 연산을 해줌
        //Optional : ifPresent()-> 값이 있는것만 찍어줌 이메소드사용하기위해서 Optional 자료구조 사용
       ...
       // rst1.ifPresent(System.out::println);
        System.out.println(rst1.get());


        //요소의 검색 : findFirst(), findAny() : 해당 스트림에서 첫번재 요소를 반환
        IntStream stream4 = IntStream.of(4,2,7,3,5,1,6);
        IntStream stream5 = IntStream.of(4,2,7,3,5,1,6);
        OptionalInt rst2 = stream4.sorted().findFirst(); //정렬후 첫번째값
        System.out.println(rst2.getAsInt());
        OptionalInt rst3  = stream5.sorted().findAny(); //병렬처리 관련 내용 포함되어 있어서 요소의 첫번재 값이 아닌 경우가 있음
        System.out.println(rst3.getAsInt());

        //요소의 검사  :
        //anyMatch() : 해당 스트림의 일부요소가 조건의 만족하면 true
        //allMatch() : 해당 스트림의 모든요소가 만족하면 true
        ...
        System.out.println(stream7.allMatch(n-> n > 9)); //false
        System.out.println(stream8.noneMatch(n-> n >90)); //false

        // 요소와 통계 : count(), max (), min()
